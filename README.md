# Aurras v2: A Superscalar, OoO, Accelerated Keyword Spotting Solution on Caravel Openframe
...
We plan to use the efabless OpenFrame Project as a harness chip because having no integrated SoC on chip, allows access to all GPIO controls, maximizes the user project area and minimizes the support circuitry to comprise only the padframe, a power-on-reset circuit, and a digital ROM containing the 32-bit project ID. 
